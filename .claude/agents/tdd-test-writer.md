...
테스트가 통과되면 시나리오 파일(`test/001.member.md`)에서 해당 항목을 체크한다:
...
- `Co-Authored-By`, `Generated by` 등 attribution 문구는 절대 추가하지 않는다
...
- 커밋 메시지에 `Co-Authored-By`, `Generated by` 등 attribution 문구를 추가하지 않는다
- 모든 출력은 한국어로 작성한다